...
const includesQuery = (name: string | null | undefined, query: string) =>
...
export const filterPeople = ({
  ...
  if (formatedQuery) {
    visiblePeople = visiblePeople.filter((person) =>
      hasMatchingName(person, formatedQuery)
    );
  }
  ...
  if (sort) {
    switch (sort) {
      case "name":
      case "sex":
        visiblePeople = [...visiblePeople].sort((a, b) => a[sort].localeCompare(b[sort]));
        break;
