...
1. `NameFilter` should update the `query` search param with the text from the input.
    - show only people with the `name`, `motherName` or `fatherName` ерфе  the query case insensitive;
...
- Replace `<your_account>` with your Github username in the [DEMO LINK](https://OlgaStakhova.github.io/react_people-table-advanced/) and add it to the PR description.
