...
+. `NameFilter` should update the `query` search param with the text from the input.
...
+. `CenturyFilter` should allow to choose several centuries or all of them.
    - add `centuries` search params using `append` method  `getAll` method;
+. Implement sorting by `name`, `sex`, `born` and `died` by clicking on arrows in a `th`;
...
[DEMO LINK](https://IzzetYusufov.github.io/react_people-table-advanced/) and add it to the PR description.
